...
import { OramaData } from '@orama/plugin-docusaurus-v3/dist/types.js'
import { getColorMode, getPreferredVersion } from './utils'
...
const extraSearchProps = {
  resultMap: {
    description: (item: any) => {
      const clean = item.path
        .replace('/personal-wiki/docs/', '')
        .split('#')[0]
        .replace(/\/+$/, '')
...
export function OramaSearchNoDocs() {
  ...
  const closeOnResultClick = () => {
    if (searchBoxRef.current) {
      searchBoxRef.current.open = false
    }
  }
  ...
  return (
    <React.Fragment>
      ...
      <OramaSearchBox
        ...
        onSearchResultClick={closeOnResultClick}
        searchParams={{
